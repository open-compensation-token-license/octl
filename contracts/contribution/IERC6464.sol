...
// Creator Events -  Copyright 2024, Tim Frey, Christian Schmitt
// License Open Compensation Token License https://github.com/open-compensation-token-license/license
interface CreatorEvents {
    /**
     * @notice Emitted when approval is explicitly granted or revoked for a token.
     */
    event ExplicitCreatorApprovalFor(
        address indexed operator,
        uint256 indexed tokenId,
        bool approved
    );
     ...
     * @dev MUST be emitted upon calls to `revokeAllExplicitApprovals()`.
     */
    event AllExplicitCreatorApprovalsRevoked(address indexed owner);
     ...
     * the `from` address expected to be emitted in the respective `ERC721.Transfer()` event.
     */
    event AllExplicitCreatorApprovalsRevoked(
        address indexed owner,
        uint256 indexed tokenId
    );
    event CreatorApprovalForAll(
        address indexed owner,
        address indexed operator,
        bool approved
    );
